...
{
    public class UpdateCategoryHandler : IRequestHandler<UpdateCategoryCommand, UpdateCategoryCommandDTO>
    {
        #region Fields
        ...
        public UpdateCategoryHandler(IUnitOfWork unitOfWork, IMapper mapper, ICategoryRepository CategoryRepository, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<UpdateCategoryCommandDTO> Handle(UpdateCategoryCommand request, CancellationToken cancellationToken)
        {
            try
            {
                var CategoryResponse = new UpdateCategoryCommandDTO();
                var requestModel = request.Id;
                var convertToCategoryDO = _mapper.Map<CategoryDO>(request);

                string propertiesToInclude = "";
                var response = await _CategoryRepository.ReadByIdAsync(request.Id, propertiesToInclude, cancellationToken);
                if (response != null)
                {
                    ...
                    var result = await _CategoryRepository.Update(response, cancellationToken);

                    //await _unitOfWork.SaveChangesAsync(cancellationToken);
                    //await _unitOfWork.CommitTransactionAsync(cancellationToken);
                    var convertToCategoryDtO = _mapper.Map<UpdateCategoryCommandDTO>(result);
