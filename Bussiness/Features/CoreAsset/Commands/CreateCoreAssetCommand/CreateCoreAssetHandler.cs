...
{
    public class CreateCoreAssetHandler
        : IRequestHandler<CreateCoreAssetCommand, CreateCoreAssetCommandDTO>
    {
        #region Fields
        ...
        public CreateCoreAssetHandler(IUnitOfWork unitOfWork, IMapper mapper, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<CreateCoreAssetCommandDTO> Handle(CreateCoreAssetCommand request, CancellationToken cancellationToken)
        {
            try
            {
                CoreAssetDO CoreAsset = _mapper.Map<CoreAssetDO>(request);
                ...
                CoreAsset.CreatedBy = empCode.ToString();
                CoreAsset.CreatedOn = DateTime.Now;

                var result = await _unitOfWork.CoreAssetRepository.Create(CoreAsset, cancellationToken);

                await _unitOfWork.SaveChangesAsync(cancellationToken);
                await _unitOfWork.CommitTransactionAsync(cancellationToken);

                return _mapper.Map<CreateCoreAssetCommandDTO>(result);
            }
            catch (Exception ex)
            {
